...
// 											const hasExistingFile = lastCertificate?.existingFile
											
// 											// Запись считается заполненной если есть тип сертификата И (загруженный файл ИЛИ файл с бэкенда)
...
import { UploadOutlined, PlusOutlined, MinusCircleOutlined, ExclamationCircleOutlined } from '@ant-design/icons'
import { Button, Checkbox, Col, Form, Modal, Result, Row, Spin, Tooltip, Upload, message } from 'antd'
import { Select } from 'antd'
import Title from 'antd/es/typography/Title'
import { t } from 'i18next'
...
const Languages = () => {
	...
	const [selectId, setSelectId] = useState<string | number | null>(null)
	const [certificateFiles, setCertificateFiles] = useState<{[key: number]: File}>({})
	const [isFormDirty, setIsFormDirty] = useState(false) // Новое состояние
	const { data: dataNative, isLoading: isFetchingNative, refetch, isError } = useGetNativeLanguagesQuery()
	const { data: dataLevels } = useGetLevelsQuery()
	const { data: dataCertificate } = useGetCertificateQuery()
	const { data: dataAll } = useGetAllNativeLanguagesQuery()
	...
	const handleFormChange = () => {
		...
		const hasLanguage = formValues.languageCode
		const hasLevel = formValues.languageLevelCode
		const hasPublished = formValues.isPublished
		const hasCertificates = formValues.certificates?.some((cert: any) => 
			cert?.certificateTypeId || certificateFiles[Object.keys(certificateFiles).find(key => Number(key) >= 0) as any]
		)
		const hasFiles = Object.keys(certificateFiles).length > 0

		setIsFormDirty(hasLanguage || hasLevel || hasPublished || hasCertificates || hasFiles)
	}
	...
	const beforeUpload = (file: File, fieldIndex: number) => {
		...
		setIsFormDirty(true) // Отмечаем форму как измененную
