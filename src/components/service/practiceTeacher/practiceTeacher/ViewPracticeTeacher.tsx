...
export const ViewPraciceTeacher = () => {
  ...
  const [isModalOpen, setIsModalOpen] = useState(false)
  const {
    data: { students, isSentToDekanat, isReportExist } = {},
    isSuccess: isSuccessOrder,
    isFetching: isFetchingMyPractice,
    refetch: refetchAll,
  } = useGetOneGroupQuery(id, { skip: !id })
  const isMobile = isMobileDevice()
  const [dataTable, setDataTable] = useState<any>(students)
  const { data: dataChat, isFetching: isFethcingChat, refetch } = useGetChatQuery(idStudent, { skip: !idStudent })
  const [sendMessageApi, { isLoading }] = useSendMessageMutation()
  ...
  const {
    ...
    isSuccess: isSuccessCompetences,
    isFetching: isFetchingComp,
    refetch: refechComp,
  } = useGetCompetencesQuery(
    { studentId: idStudent, orderId: id },
    { skip: !idStudent, refetchOnMountOrArgChange: true },
  )
  const dispatch = useAppDispatch()
  const [oldId, setOldId] = useState(null)
  const { data: dataReportStudent } = useGetInfoReportStudentQuery(idStudent, { skip: !idStudent })
  ...
  const columnsMini = [
        ...
        return (
          <>
            ...
            <div className="!text-xs">
              ...
              {text?.isThirdDocSent ? "Отправлено" : "Не отправлено"}
            </div>
          </>
        )
      },
    },
    {
      key: "report",
      dataIndex: "report",
      title: t("reportAvailability"),
      className: "text-xs !p-4 mobileFirst",
      render: (record: any, text: any) => {
        return (
          <>
            {text?.isReportExist ? (
              <Button onClick={() => modalButton(text)} className="p-5 w-48">
                {t("reportGenerated")}
              </Button>
            ) : (
              <Button onClick={() => modalButton(text)} className="p-5 w-48">
      ...
      <Spin spinning={isFetchingMyPractice}>
        <section className="container animate-fade-in">
          <Space size={10} align="center">
            ...
            />
            <span className=" text-[28px] font-normal">{t('practiceGroup')} {students?.[0]?.groupNumber}</span>
          </Space>

          <Row gutter={[16, 16]} className="mt-14  flex items-center">
            <Col span={5}>
              <span>{t("fioStudent")}</span>
            </Col>
            <Col span={7} className="overWrite">
              <Form.Item className="mb-0" name={"name"}>
                <Select
            ...
            <Col className="flex md:justify-end" span={12}>
              <Button onClick={openReportModal}>{t("formReport")}</Button>
            </Col>
          </Row>
          <Row gutter={[16, 16]} className="mt-4 flex items-center">
            <Col span={5}>
            ...
            <Col span={7} className="overWrite">
              <Form.Item className="mb-0" name={"grade"}>
                <Select
                  ...
                  popupMatchSelectWidth={false}
                  className="w-full"
                  options={[{ value: "Все", label: t("all") }].concat(optionMockGrade)}
                  onChange={(value: any) => {
                    setFilter({ ...filter, grade: value })
                  }}
                />
            ...
            <Col className="flex md:justify-end" span={12}>
              <Tooltip title={true === true ? t("formReport") : ""}>
                <Button onClick={() => sendDekanat(id)} disabled={!isReportExist}>
                  {t("sendDean")}
                </Button>
              </Tooltip>
            </Col>
            ...
            <Col span={7} className="overWrite">
              <Form.Item className="mb-0" name={"status"}>
                <Select
                  ...
                  popupMatchSelectWidth={false}
                  className="w-full"
                  options={optionMockStatus.map((item) => ({
                    ...item,
                    label: item.label === "Все" ? t("all") : item.label,
                  }))}
                  onChange={(value: any) => {
                    setFilter({ ...filter, status: value })
                  }}
          ...
          <Row className="mb-6 !flex">
            <Col span={12}>
              <div>
                <Col className="!flex gap-3" span={12}>
                  <span className="text-gray-500">{t("statusGeneration")}: </span>
                  <span>{isReportExist ? <a onClick={downloadTwo}>{t("formed")}</a> : t("notFormed")}</span>
                </Col>
                <Col className="flex gap-3" span={12}>
                  <span className="text-gray-500">{t("statusSendingDean")}: </span>
                  <span>{isSentToDekanat ? t("sent") : t("notSent")}</span>
                </Col>
              </div>
            </Col>
            <Col className="flex justify-end" span={12}>
          ...
          <Drawer
            ...
            title={
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span>{studentName}</span>
                <div className="flex gap-3">
                  <Button type="primary" onClick={() => openStudentModal(studentName)}>
                    Отчет
                  </Button>
                  <Button type="text" icon={<CloseOutlined />} onClick={onClose} />
                </div>
              ...
              <div className="top-10">
                <Spin spinning={isLoadingUpdateStatus}>
                  <Form form={form} className="flex  w-full flex-wrap " onFinish={onFinish}>
                    ...
                    {grade === "освобожден" ||
                    grade === "Отсутствие по уважительной" ||
                    grade === "Отсутствие по неуважительной" ? (
                      <Row className="flex items-center w-full mt-4">
                        <Col span={5}>Причина</Col>
                        <Col span={19}>
                          <Form.Item className="w-full mb-0" name={"reasonText"}>
                            <Input placeholder={"Напишите причину"} maxLength={20} className="w-full" />
                          </Form.Item>
                        </Col>
                      </Row>
                    ) : (
          ...
          <ModalReport
            id={id}
            setIsModalOpenReport={setIsModalOpenReport}
            handleOk={handleOk}
            openModalReport={openModalReport}
          />

          <ModalStudent
            dataReportStudent={dataReportStudent}
            students={students}
            isFetchingComp={isFetchingComp}
            id={id}
            idStudent={idStudent}
            isSuccessCompetences={isSuccessCompetences}
            data={dataCompetences}
            dataCompetences={dataCompetences?.competencesTable}
            rowData={rowData}
            setIsModalStudent={setIsModalStudent}
            handleOk={handleOk}
