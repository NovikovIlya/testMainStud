...
export const ViewRepresentation = () => {
	...
	const columns = [
		...
		},
		{
			key: 'FIO',
			dataIndex: 'FIO',
			title: t('departmentDirectorName'),
			className: 'text-xs !p-2 mobileFirst',
			render: (text: any, record: Submission) => <span >{record?.practice?.departmentDirector}</span>
		},
		{
			...
			className: 'text-xs !p-2 mobileFirst',
			render: (text: any, record: Submission) => <span >{record?.isWithDeparture ? t('yes') : t('no')}</span>
		},
		{
			key: 'status',
			dataIndex: 'status',
	...
	return (
		...
		<section className="container animate-fade-in">
				...
				<Form.Item className='mb-0 ' name={'visiting'}>
					<Select
						...
						options={[
							...
							{ value: 'Да', label: t('yes') },
							{ value: 'Нет', label: t('no') },
						]}
						onChange={(value: any) => {
							setFilter({ ...filter, visiting: value })
						}}
			...
			<Row gutter={[16, 16]} className="mt-4 flex items-center">
				<Col span={5} >
					<span>{t('departmentDirectorName')}</span>
				</Col>
				<Col span={7} className='overWrite'>
					<Form.Item className='mb-0'  name={'FIO'}>
					<Select
					...
					<Select
						...
						options={[
							...
							...(dataSubmissionAcademicYear
								? dataSubmissionAcademicYear?.map((item: any) => ({
										key: item.id,
										value: item.value,
										label: item.label
								  }))
