...
// }
import { LoadingOutlined } from '@ant-design/icons'
import { Button, Col, ConfigProvider, Form, Radio, Row, Select, Space, Spin, Table, Tooltip, TreeSelect } from 'antd'
import dayjs from 'dayjs'
import { useEffect, useState } from 'react'
...
export const ViewAll = () => {
	...
	const { data: dataPracticeType, isSuccess: isSuccessPracticeType } = useGetSubmissionsTypePracticeQuery(
		subDevisionId,
		{
			skip: !subDevisionId || subDevisionId === t('all')
		}
	...
	const optionYears = [{ value: t('all') }, { value: t('current') }, { value: t('past') }]

	const optionYearsTwo = [{ value: t('newestFirst') }, { value: t('oldestFirst') }]
				...
				<Form form={form} initialValues={initialFormValues}>
					...
					<Row gutter={[16, 16]} className="mt-4 overWrite items-center">
						<Col span={2} className="flex items-center overWrite">
							<span className="w-12">{t('courseNumber')}</span>
						</Col>
						<Col span={2} className="flex items-center overWrite">
							<Form.Item name={'course'} className="mb-[-4px] w-auto sm:w-full overWrite">
								<Select
