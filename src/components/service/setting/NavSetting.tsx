...
export const NavSetting = () => {
  ...
  const navItems = [
    ...
    {
      key: '/services/setting/changePassword',
      icon: <ChangePasswordSvg />,
      label: <p className="ml-[10px]">Изменить пароль</p>,
      // Пример скрытия пункта для определённой роли:
      hidden: mainRole === 'OTHER',
    },
    ...
    <>
      ...
      <div className="flex w-full">
        ...
        <div className="ml-[229px] bg-[#F5F8FB] w-full pt-[70px] min-h-screen">
          {pathname === '/services/setting/contactInformation' && <MainContact />}
          {pathname === '/services/setting/changePassword' && <ChangePassword />}
        </div>
      </div>
    </>
  )
