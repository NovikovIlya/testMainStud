...
export const EditRepresentation = () => {
	...
	const downloadFile = () => {
		...
		if (dataGetDocRepresentation) {
			const link = document.createElement('a')
			link.href = dataGetDocRepresentation
			link.setAttribute('download', `${t('representationInOrder')}.docx`)
			document.body.appendChild(link)
			link.click()
	...
	const editData = () => {
		if(selectedPlace==='В профильной организации'){
			if(fullTable.some((item:any)=>item.place===null)){ 
				return dispatch(showNotification({ message: t('fillPracticeMessage', { extraInfo: dataOneSubmissions.isWithDeparture ? `,  "${t('dailyAllowance')}", "${t('travel')}" и "${t('accommodation')}"` : '' }), type: 'warning' }));
			}
			if(dataOneSubmissions.isWithDeparture){
				if(fullTable.some((item:any)=>item.costForDay===null || item.arrivingCost===null || item.livingCost===null)){ 
					return dispatch(showNotification({ message:  t('fillPracticeMessage', { extraInfo: dataOneSubmissions.isWithDeparture ? `,  "${t('dailyAllowance')}", "${t('travel')}" и "${t('accommodation')}"` : '' }), type: 'warning' }));
		...
		editSumbissions(obj[0])
			...
			.then(() =>{
				dispatch(
					showNotification({
						message: t('representationEditSuccess'),
						type: 'success'
					})
				)
				refetch()
		...
		<section className="container animate-fade-in">
			<Row gutter={[16, 16]}>
				<Col span={24}>
					...
					/>
					<Typography.Text className=" text-[28px] mb-14">
						{t('representationForGroup')} "
						{dataOneSubmissions?.practice.groupNumber}" {t('subdivision')} "
						{dataOneSubmissions?.practice.subdivision}" {t('for')}
						{dataOneSubmissions?.practice.academicYear} {t('academicYear')}
					</Typography.Text>
				</Col>
			</Row>
			<Descriptions className="mt-12" items={items} />
			...
			<Row className="mt-4 mb-6 flex  justify-between">
				...
				<Col span={12} className="flex justify-end">
							...
							<Button disabled={dataOneSubmissions.status === 'Согласован'} onClick={handleChangeStatus}>{t('agreeRepresentation')}</Button>
