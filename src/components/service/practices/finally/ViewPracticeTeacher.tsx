...
export const ViewPraciceTeacher = () => {
  ...
  // Используем константы для значений фильтров (языконезависимые)
  const ALL_VALUE = 'all'
  ...
  const [filter, setFilter] = useState({
    name: ALL_VALUE,
    courseNumber: ALL_VALUE,
    dateFilling: NEWEST_FIRST, // "По дате (сначала новые)"
    status: ALL_VALUE,
    grade: ALL_VALUE
  })
  const {
    ...
    refetch: refetchAll
  } = useGetAllStudentsFinalQuery(id, { refetchOnMountOrArgChange: true })
  const [dataTable, setDataTable] = useState<any>(students)
  const { data: dataReportCurrentStudent, isError: isErrorStudent, isFetching: isFetchingStudent } = useGetInfoReportStudentQuery(idStudent?.id, { skip: !idStudent.id })
  const { data: dataReportGurrentGroup, isFetching: isFetchingGroup } = useGetInfoReportGroupQuery(idGroup?.id, { skip: !idGroup.id })
  const [objAttachment, setObjAttachment] = useState<any>({ studentId: null, documentType: null, count: 0 })
  ...
  // Опции для оценок (переводимые строки)
  const optionMockGrade = [
    { label: '0', value: '0' },
    { label: t('notPassed'), value: t('notPassed') },
    { label: '2', value: '2' },
    { label: '3', value: '3' },
    { label: '4', value: '4' },
    { label: '5', value: '5' },
    { label: t('passed'), value: t('passed') },
    { label: t('exempt'), value: t('exempt') },
    { label: t('absenceExcused'), value: t('absenceExcused') },
    { label: t('absenceUnexcused'), value: t('absenceUnexcused') }
  ]
  ...
  const columnsMini = [
    ...
    },
    {
      key: 'documentsStudent',
      dataIndex: 'documentsStudent',
      title: t('studentDocuments'),
      className: 'sm:text-xs !p-4 mobileFirst',
      render: (record: any, text: any) => {
        return (
          <>
            <div className="!text-xs">
              {t('report')}: {text?.isReportSent ? <a onClick={() => downloadlChat(text.id, 'report')}>{t('downloadDoc')}</a> : t('notSent')}
            </div>
            <div className="!text-xs">
              {t('diary')}: {text?.isDiarySent ? <a onClick={() => downloadlChat(text.id, 'diary')}>{t('downloadDoc')}</a> : t('notSent')}
            </div>
            <div className="!text-xs">
              {(text?.thirdDocType === 'TASKS' ? t('individualTasks') : t('voucher'))}:{' '}
              {text?.isThirdDocSent ? <a onClick={() => downloadlChat(text.id, 'tasks')}>{t('downloadDoc')}</a> : t('notSent')}
            </div>
          </>
        )
      }
    },
    {
      ...
      render: (record: any, text: any) => {
        return (
          <a onClick={() => downloadCurrenReportStudent(text.id)} className="text-base">
            {t('reportDoc')}
          </a>
        )
      }
  ...
  useEffect(() => {
    if (dataReportCurrentStudent) {
      download(dataReportCurrentStudent, t('studentReportFileName', { studentName }))
    }
  }, [dataReportCurrentStudent])

  useEffect(() => {
    if (dataReportGurrentGroup) {
      downloadTwo(dataReportGurrentGroup, t('groupReportFileName'))
    }
  }, [dataReportGurrentGroup])

  useEffect(() => {
    if (dataAttachement) {
      downloadThree(dataAttachement, t('chatReportFileName'))
    }
  }, [dataAttachement])

  useEffect(() => {
    if (isErrorAttachments)
      dispatch(showNotification({ message: t('documentNotSubmitted'), type: 'error' }))
  }, [isErrorAttachments])

  useEffect(() => {
    if (isErrorStudent)
      dispatch(showNotification({ message: t('reportNotGenerated'), type: 'error' }))
  }, [isErrorStudent])
  ...
  const downloadTwo = async (url: any, name: any) => {
    if (dataReportGurrentGroup) {
      ...
      document.body.appendChild(link)
      link.click()
      dispatch(showNotification({ message: t('fileDownloaded'), type: 'success' }))
    }
  }
  ...
  const downloadCurrenReportStudent = (idz: any) => {
    if (!isErrorStudent) {
      if (idz === idStudent.id) {
        download(dataReportCurrentStudent, t('studentReportFileNameShort'))
        return
      }
    }
    setIdStudent((prev: any) => ({ ...prev, id: idz, count: Math.random() }))
  ...
  const downloadCurrenReport = () => {
    if (idGroup.id === id) {
      downloadTwo(dataReportGurrentGroup, t('groupReportFileNameShort'))
      return
    }
    setIdGroup((prev: any) => ({ ...prev, id: id, count: Math.random() }))
  }
  ...
  function filterDataFull() {
    function filterCourse(elem: any) {
      if (filter.courseNumber === ALL_VALUE) {
        return elem
      } else {
        return elem.course === filter.courseNumber
      }
    ...
    function filterStatus(elem: any) {
      if (filter.status === ALL_VALUE) {
        return elem
      } else if (filter.status === PENDING_REVIEW) {
        return elem.status === null || elem.status === PENDING_REVIEW
      } else {
        return elem.status === filter.status
      }
    }
    function filterName(elem: any) {
      if (filter.name === ALL_VALUE) {
        return elem
      } else {
        return elem.studentName === filter.name
      }
        ...
        <section className="container animate-fade-in">
          <Space size={10} align="center">
            ...
            <span className="text-[28px] font-normal">
              {t('groupPractice', { group: students?.[0]?.group })}
            </span>
          </Space>
          ...
          <Row gutter={[16, 16]} className="mt-14 flex items-center">
            ...
            <Col className="flex md:justify-end" span={12}>
              <Button onClick={downloadCurrenReport}>{t('downloadReport')}</Button>
            </Col>
          </Row>
          <Row gutter={[16, 16]} className="mt-4 flex items-center">
            <Col span={5}>
            ...
            <Col span={7} className="overWrite">
              <Form.Item className="mb-0" name={'status'}>
                <Select
                  ...
                  popupMatchSelectWidth={false}
                  className="w-full"
                  options={[{ value: ALL_VALUE, label: t('all') }].concat(optionMockGrade)}
                  onChange={(value: any) => {
                    setFilter({ ...filter, grade: value })
                  }}
                />
          ...
          <Row>
            <Col className="flex justify-end" span={24}>
              <Button onClick={refetchAll} size="large" shape="circle" icon={<ReloadOutlined />} />
            </Col>
          </Row>
            ...
            <Col flex={'auto'}>
              {!fullTable ? (
                ...
                <Table
                  ...
                  size="large"
                  rowKey="id"
                  // Если fullTable, предположим, используются другие колонки
                  columns={[]}
                  dataSource={dataTable.map((item: any, index: number) => ({ ...item, number: index + 1 }))}
                  pagination={
                    dataTable.length > 10 && {
