...
export const EditAppendix = () => {
  ...
  const { data: dataOneSubmissions, isSuccess, isLoading: isLoadingOneSubmission } =
    useGetOneApplicationQuery(id, { skip: !id });
  const {
    data: dataGetDocApplication,
    isLoading: isLoadingDocApplication,
    refetch
  } = useGetDocApplicationQuery(
  ...
  useEffect(() => {
    if (isSuccess) {
      const x = {
        value: dataOneSubmissions.contract.id,
        label: `${dataOneSubmissions.contract.contractFacility} №${dataOneSubmissions.contract.contractNumber} от ${dayjs(
          dataOneSubmissions.contract.conclusionDate
        ).format('DD.MM.YYYY')}`
      };
      setSelectDogovor(x);
    }
  ...
  useEffect(() => {
    if (isSuccess) {
      const array = dataOneSubmissions.students.map((item: any) => ({
        ...
        other: (
          <>
            <div>{dataOneSubmissions.practice.courseNumber} курс,</div>
            <div>
              {dataOneSubmissions.practice.practiceKind},
              <div></div>
              {dataOneSubmissions.practice.practiceType}
            </div>
          </>
        )
      }));
  ...
  const downloadFile = () => {
    if (dataGetDocApplication) {
      ...
      link.setAttribute(
        'download',
        `Приложение к договору группы "${dataOneSubmissions?.practice.groupNumber}" подразделения "${dataOneSubmissions?.practice.subdivision}" на ${dataOneSubmissions?.practice.academicYear} учебный год.docx`
      );
      document.body.appendChild(link);
      link.click();
    }
  ...
  const editData = () => {
    ...
    editApplication({ id: dataOneSubmissions.id, body: obj })
      .unwrap()
      .then(() => {
        dispatch(
          showNotification({
  ...
  return (
    <section className="container animate-fade-in">
      ...
      <Row>
        <Col span={24}>
          <Typography.Text>{t('contract')}</Typography.Text>
        </Col>
        <Col className="mt-2" span={12}>
          <Select
            style={{ width: '100%' }}
            showSearch
            value={selectDogovor}
            placeholder={t('selectContractPlaceholder')}
            onChange={handleChange}
            filterOption={(input, option) =>
              (option?.label ?? '')
                .toString()
        ...
        <Col span={12}>
          <Space>
            {!isEdit ? (
              ...
              <Popover content={t('downloadHint')}>
                <Button
                  onClick={downloadFile}
                  loading={isLoadingDocApplication}
                  disabled={isLoadingDocApplication || isEdit}
      ...
      <Row className="mt-4">
        <Col flex={'auto'}>
          <Form form={form} component={false}>
            <TableEditView
              visiting={dataOneSubmissions?.isWithDeparture}
              fullTable={fullTable}
