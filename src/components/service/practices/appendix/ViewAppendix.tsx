...
import { PopoverContent } from './PopoverContent';
import { PopoverMain } from './PopoverMain';
import { useGetAcademicApplicationQuery, useGetAllApplicationsQuery, useGetAllSubmissionsQuery, useGetSubmissionsApplicationQuery, useGetSpecialtiesApplicationQuery, useGetPracticeTypeApplicationQuery, useGetPracticeKindApplicationQuery, useGetDirectorApplicationQuery } from '../../../../store/api/practiceApi/representation';
import { LoadingOutlined } from '@ant-design/icons';
import { findSubdivisions } from '../../../../utils/findSubdivisions';
import { disableParents } from '../../../../utils/disableParents';
...
export const ViewAppendix = () => {
	...
	const treeData = [
		...(dataSubmisisionsApplication ? dataSubmisisionsApplication.map((item: any) => {
			return {
				title: item.value,
				value: item.id,
				// @ts-ignore
							...
							<ConfigProvider
								...
								locale={{
									emptyText: (
										<div>
											<h3>{t('noData')}</h3>
											{!selectSubdivisionId ? <p>{t('subdivisionEmptyWarning')}</p> : ''}
										</div>
									)
								}}
							/></ConfigProvider>
