...
import { useInterval, useUpdateEffect } from 'ahooks'
import { Button, Card, Form, Input, Modal, Row, Spin, Tooltip } from 'antd'
import Title from 'antd/es/typography/Title'
import { t } from 'i18next'
import { useEffect, useState } from 'react'
...
export const ContactInformation = () => {
	...
	const dispatch = useAppDispatch()
	const mail = useAppSelector(state => state.auth.user?.email)
	const textForm = Form.useWatch('inputText', form)
	...
	const onSubmitPhone2 = () => {
		...
		postMail({ email: text, marker: 'PRIVATE' })
			.unwrap()
	...
	return (
		<Form form={form} onFinish={onSubmitPhone2} className="w-full">
			<Row className="mb-8 flex items-center justify-between mt-6">
				<Title className="!mb-0 !text-[28px]">{t('contactInformation')}	
						
					</Title>
				<Tooltip title={
					<>
					<div className='!text-yellow-50'>{t('textForContactInformation')}</div>
					<div className='!text-yellow-50 font-extrabold'>123 <img className='rounded-[50%]' src='https://i.postimg.cc/13gRWSd6/image.png'/></div>
