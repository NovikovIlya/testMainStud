...
const Diary = ({id,dataDiary,setShowFinalTwo}:any) => {
	...
	const download = async ()=>{
		const link = document.createElement('a')
		link.href = data
		link.setAttribute('download', `${t('diaryFileName')}.docx`)
		document.body.appendChild(link)
		link.click()
	}
	...
	return (
		...
		<Spin style={{width:'50%'}} className='w-full sm:w-[50%] flex flex-wrap' spinning={isLoading} >
			...
			<Row>
				<Col>
					<Typography.Title level={2}>{t('diary')}</Typography.Title>
				</Col>
			</Row>
			<Row className=''>
				<Col  xs={24} sm={24} md={24} >
			...
			<Row gutter={[16, 16]} className="my-8">
				<Col xs={24} sm={24} md={24} lg={24} xl={24}>
					<Space className="w-full">
					<Popover content={isDisabled ? t('diaryPopover') : null}><Button
							className="!rounded-full text-[10px] sm:text-base"
							size="large"
                            disabled={isDisabled}
                            onClick={handleSave}
						...
						>
							<VerticalAlignBottomOutlined />	{t('saveAndDownloadDiary')}
						</Button></Popover>
					</Space>
				</Col>
			</Row>
