...
import {
	CompressedIndividualTask,
	FullIndividualTask
} from '../../individual-tasks/IndividualTasks'
...
export const IndTaskPopoverContent = ({
	...
	function translateColumnIntoRussia({ isPrint }: { isPrint: boolean }) {
		if (recordCompressed) {
			return {
				...
				'Дата заполнения': dayjs(recordCompressed.dateFilling).format(
					'DD.MM.YYYY'
				),
	...
	function downLoad() {
		const ws = utils.json_to_sheet([
			translateColumnIntoRussia({ isPrint: false })
		])
