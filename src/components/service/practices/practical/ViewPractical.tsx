...
export const ViewPractical = () => {
	...
	const optionsSortDate: any = [
        {value: 'По дате (сначала новые)', label: t('newestFirst')},
        {value: 'По дате (сначала старые)', label: t('oldestFirst')},
    ]
	...
	const columnsCompressed = [
		...
		{
            title: <TitleHeadCell title={t('sortDate')}/>,
            dataIndex: 'dateFilling',
            width: '20%',
            render: (text:any) => dayjs(text).format('DD.MM.YYYY')
        },
		...
		},
		{
			key: 'courseNumber',
			dataIndex: 'courseNumber',
			title: <TitleHeadCell title={t('courseStudy')}/>,
			align: 'center',
			className: 'text-xs !p-2 mobileFirst'
		},
		{
	...
	const treeData = [{ key: 'Все', value: 'Все', label: t('all') },...(dataSubdevisionPracticeNew ? dataSubdevisionPracticeNew?.map((item:any)=>{
        return{
            title:item.value,
            value:item.id,
            // @ts-ignore
	...
	return (
		<>
			...
			<Form form={form} initialValues={initialFormValues}>
				<Row>
					<Col flex={'auto'}>
						<span className="mb-14 text-[28px]">{t('practical')}</span>
					</Col>
				...
				<Row gutter={[16, 16]} className="mt-4 overWrite items-center ">
					<Col span={4} className='flex items-center overWrite'>
						<span className='w-12'>{t('courseStudy')}</span>
